...
public class ServerFunctionExecuter{
	...
	public Worker SignIn(String i_WorkerNumber,String i_Password) throws IOException, FromServerException,
				JSONException, CreateWorkerException{
	...
	public void ChangePurchaseTrack(String i_CustomerType, double i_Discount) throws IOException, FromServerException{
		...
		RequestMessage requestMessage = new RequestMessage(eFunctions.ChangePurchaseTrack,purchaseTrackToChange.GetJSONObject());
		Execute(requestMessage);
	}
	...
	public Vector<Person> GetAllCustomers() throws JSONException, IOException,
				FromServerException, CreateCustomerException, CreateWorkerException{
		return GetAllPerson(eFunctions.GetAllCustomers);
	}
	
	public Vector<Person> GetAllWorkers() throws JSONException, IOException,
				FromServerException, CreateCustomerException, CreateWorkerException{
		return GetAllPerson(eFunctions.GetAllWorkers);
	}
	
	public Vector<Person> GetAllPerson(eFunctions i_Function) throws IOException, FromServerException, JSONException,
				CreateCustomerException, CreateWorkerException{
	...
	public Map<eItemCategory,ItemAmount> GetTodaysSalesReportByCategorys(String [] i_Categorys) throws IOException,
				FromServerException{
		...
		for(int i = 0; i < resultJSON.length(); i++){
			...
			if(report.containsKey(current.getItemCategory())){
				report.get(current.getItemCategory()).setAmount
								(report.get(current.getItemCategory()).getAmount() + current.getAmount());
			}
			else{
	...
	public Map<Customer,Vector<ItemAmount>> GetTodaySalesReporyByBranch(String [] i_BranchsName) throws IOException, FromServerException, 
				JSONException, CreateCustomerException{	
		...
		for(int i = 0; i < resultJSON.length(); i++){
			...
			currentCustomer = CustomerFactory.CreateCustomer(resultJSON.getJSONObject(i).getJSONObject("buyer"));
